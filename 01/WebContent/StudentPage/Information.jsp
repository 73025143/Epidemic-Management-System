...
<%  
	   ...
	   if(session.getAttribute("seterror") != null) {  
		   seterror =Integer.valueOf(session.getAttribute("seterror").toString());
	   }  
 %> 
<!DOCTYPE html>
<html>
...
<title>Information Setting</title>
     <% String StudentID= session.getAttribute("StudentIDS").toString();
   		String loginUser = session.getAttribute("loginUserS").toString();
   		String Telephone = session.getAttribute("TelephoneS").toString();  
   		String Major = session.getAttribute("MajorS").toString();  
   		String Grade = session.getAttribute("GradeS").toString();  
 	%> 
 	<%if(seterror==-1) {%>  
	   			<body onload="javascript:window.alert('Failed to change information: the phone number already exists')">
	   <%session.setAttribute("seterror","9");}%>
	  	<%if(seterror==1) {%>  
	   			<body onload="javascript:window.alert('Updated')">
	   <%session.setAttribute("seterror","9");}%>
</head>
<body>
<div class=main>
...
<div class="demo" >
            ...
            <table style="height: 300px; width: 411px;">
                 <tr>
                  <td align="left" class="style1">Student ID</td>
                ...
                <tr>
                  <td align="left" class="style1">Grade</td>
                  <td align="left" class="style2">
                      <input onkeyup="this.value=this.value.replace(/\D/g,'')" 
                          onafterpaste="this.value=this.value.replace(/\D/g,'')" name="grade" id="grade" 
                          style="width: 34px" value=<%=Grade%>></td>
                </tr>
                 <tr>
                  <td align="left" class="style1">Major</td>
                  <td align="left" class="style2"><select  name="major" id="major">
					<option value="Other">Other</option>
					<option value="CSSE" <% if ("CSSE".equals(Major)) { %>selected="selected"<%}%>>CSSE</option>
				</select></td>
